...
#define ADDR_LENGA 0 // address of the length identifier
#define ADDR_LENGB 1 // address of the copy of length identifier
#define ADDR_ID 2 // address of the ID
#define ADDR_TYPE 3 // address of the message type
#define ADDR_CONT 4 // address of the contents of the message
#define SIZE_CONST 6 // the size of the everything but the message
...
#define TEMP_PIN A1 // pin for temprature sensing
#define VOLT_PIN A0// pin for battery voltage sensing
#define MOTOR_PIN 6 // pin for writing to esc
...
String readInstruction(char *bytes, int length){
  ...
  if (bytes[ADDR_LENGA] == bytes[ADDR_LENGB]){
     ...
     toReturn = toReturn + ", Length Failed: " + bytes2Int(received,1+ADDR_LENGA,ADDR_LENGA) + '/' + bytes2Int(received, 1+ADDR_LENGB, ADDR_LENGB);
     receivedLast = 0x00;
     // lengthMistakes++;
     return toReturn;
  }
...
void actions () {
  ESC.write(map(analog,255,0,2400,900));
}
...
void loop() {
  batteryVoltage = (analogRead(VOLT_PIN)*(7.2/1.8)); //TODO edit these values /1024?
  senseTemp = analogRead(TEMP_PIN);
  milliVolts = (senseTemp/1024)*5000;
  temprature = ((milliVolts-500)/10)+7;
  ...
  if (j>13){
    ...
    if (errorCode.equals("Length Passed, Checksum Passed")){ // good message
      ...
      if (getType() == IA_EL ){ // type for control message 
        ...
        if (ledState == 1) { // if led should be on
           if (mode == 0){ // power visualisation mode
             if (analog == 127){
                color.red = 0;
                color.blue = 255;
                color.green = 0;
         ...
         runOnce = 0;
       } else if (runOnce == 0){ // if they should be off
          runOnce = 1;
          color.red = 0;
          color.blue = 0;
          color.green = 0;
    ...
    writeIntegerValue((int)temprature, 0);
    writeFloatValue(batteryVoltage, 1);
    changeInstruction(ET_IV);
    Serial.write(message,lookupSizes[ET_IV]);
